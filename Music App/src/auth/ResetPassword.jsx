...
const ResetPassword = () => {
    ...
    let handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            //!Reset password fuctionality provided by the firebase
            await sendPasswordResetEmail(__AUTH, email)
            toast.success(`Reset Password email has been sent to your registered ${email}`)
            navigate("/auth/login");
        } catch (error) {
            ...
            <article className="w-[30%] bg-gray-700 p-5 rounded-xl">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-center items-center">
                        <NavLink
                            to={"/auth/login"}
                            className={"hover:text-[#d47777] hover:underline"}
                        >
                            Cancel
                        </NavLink>
                    </div>
