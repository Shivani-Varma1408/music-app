...
const CreateAlbum = () => {
    ...
    let handleSubmit = async (e) => {
        ...
        try {
            ...
            if (albumThumbnail) {
                //convert album thumbnail (image)=>(binary)
                let albumData = new FormData();
                albumData.append("file", albumThumbnail);
                albumData.append("upload_preset", "music-albums");
                albumData.append("cloud_name", "dja9qf3pp");
                //upload album thumbnail to the cloudinary
                let respone = await fetch("https://api.cloudinary.com/v1_1/dja9qf3pp/upload",
                    {
                        method: "POST",
                        body: albumData
                    }
                );
                let albumThumbnailResult = await respone.json();
            ...
            await Promise.all(
                songDetails.map(async (song, index) => {
                    ...
                    if (song.songThumbnail) {
                        ...
                        songFileData.append("upload_preset", "music-albums");
                        songFileData.append("cloud_name", "dja9qf3pp");

                        //Upload song thumbnail to cloudinary
                        let response = await fetch("https://api.cloudinary.com/v1_1/dja9qf3pp/upload",
                            {
                                method: "POST",
                                body: songFileData,
                            }
                    ...
                    let songDataObject = {};
                    if (song.songFile) {
                        //add the song file data to the FormData()-> API
                        let songFileData = new FormData();
            ...
            //! Create a payload object using fetch->API
            let payload = {
                ...albumDetails,
            ...
            let albumCollection = collection(__DB, "music-albums");
            ...
            <main className="w-full bg-gray-700 rounded-b-md">
                <form onSubmit={handleSubmit} className="w-full py-2 px-4">
                    ...
                    <div className="flex py-2">
                        ...
                        <div className="w-[350px] flex flex-col ml-13">
                            ...
                            <input
                                ...
                                value={albumDirector}
                                onChange={handleAlbumInputChange}
                                placeholder="Enter album director name"
                            />
                        </div>
                    </div>
                                ...
                                <section className="flex justify-between">
                                    ...
                                    <aside className="flex px-8 mb-4">
                                        {songDetails.length === index + 1 && (
                                            <div className="bg-[#FF9A9A] flex items-center gap-2 hover:bg-green-600 cursor-pointer py-2 px-4 rounded" onClick={addSong}>
                                    ...
                                    <aside className="flex px-8 mb-4">
                                        {songDetails.length !== 1 && (
                                            <div className="bg-[#FF9A9A] flex items-center gap-2 hover:bg-red-600 cursor-pointer py-2 px-4 rounded" onClick={() => removeSong(index)}>
                    ...
                    <article className="my-5 w-full flex justify-center items-center">
                        <button className="w-[95%] py-2 text-lg font-semibold bg-[#FF9A9A] hover:bg-[#d47777] rounded-lg mt-2 cursor-pointer">
